...
HitCandidate getDist(vec3 point, Scene scene){
    ...
    vec3 repetitionPoint = opRepetition(point - vec3(0., 5., -10.), 15.);
    float sphereDist = sphereDistance(repetitionPoint, Sphere(vec3(0.), 5., NULL_MATERIAL));
    float torusDist = torusDistance(repetitionPoint, Torus(vec3(0.), 5., 1., NULL_MATERIAL));
