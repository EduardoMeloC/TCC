...
HitCandidate getDist(vec3 point, Scene scene){
    ...
    vec3 twistPoint = opTwist(sin(iTime)* 1., point - vec3(0., 5., -10.));
    // float sphereDist = sphereDistance(twistPoint/* - vec3(0., 5., -10.)*/, Sphere(vec3(0.), 2.75, NULL_MATERIAL));
    float torusDist = torusDistance(twistPoint, Torus(vec3(0.), 5., 1., NULL_MATERIAL));
    // float displacedSphere = opDisplace(sphereDist, point, sin(iTime) * 3.);

    if(torusDist < minDist.dist){
        minDist.dist = torusDist;
